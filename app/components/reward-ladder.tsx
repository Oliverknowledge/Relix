...
export function RewardLadderCard({
  campaignId,
  founderWallet,
  onRewardPaid,
  repository,
  ...
  useEffect(() => {
    ...
    // Deferred (.then) so the initial reward state hydrates without a
    // synchronous setState inside the effect body.
    fetch(`/api/agent/reward?${params.toString()}`)
      .then((response) => (response.ok ? response.json() : null))
      .then((data: RewardState | null) => {
        if (cancelled || !data) {
          return;
        }
        setState({
  ...
  const paidCount = state.rewards.length;
  const capped = paidCount >= REWARD_LADDER_MAX_RUNG;
  const nextRung = REWARD_LADDER_RUNGS[paidCount] ?? null;

  const payNext = async () => {
    if (!founderWallet) {
      setError("Connect a devnet wallet to receive the reward payout.");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        setError(data.error || "Could not settle the reward on-chain.");
        return;
      }
      ...
      setError("Network error while settling the reward. Try again.");
    } finally {
      setPaying(false);
    }
  };
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs font-semibold uppercase tracking-wide text-[#a1a1aa]">
            On-chain capability · reward ladder
          </p>
          <h3 className="mt-1 text-lg font-semibold text-[#18181b]">
            {specialistName} settles referral rewards on-chain
          </h3>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-[#71717a]">
            This is the one deliverable that moves real value: the agent treasury
            signs each capped payout on Solana devnet — no human approval — and
            pays the referrer wallet directly. In this demo the reward lands in
            your connected wallet so you can watch the balance rise.
          </p>
        </div>
        <span className="whitespace-nowrap rounded-full bg-[#f4f4f5] px-3 py-1 text-xs font-medium text-[#52525b]">
          {state.paidTotalSol} / {REWARD_LADDER_TOTAL_CAP_SOL} SOL paid
        </span>
      </div>

      <ol className="mt-5 grid gap-2">
        {REWARD_LADDER_RUNGS.map((rung) => {
          const paidRecord = state.rewards.find(
            (reward) => reward.rung === rung.rung
          );
          const isNext = !paidRecord && rung.rung === paidCount + 1;
          ...
          return (
            ...
            >
              ...
              <span className="flex items-center gap-3">
                <span className="tabular-nums">{rung.amountSol} SOL</span>
                {paidRecord ? (
                  <a
        ...
        >
          {capped
            ...
            : paying
              ? "Settling on-chain…"
              : nextRung
                ? `Pay reward ${nextRung.rung}/${REWARD_LADDER_MAX_RUNG} (${nextRung.amountSol} SOL)`
                : "Pay next reward"}
        </button>
        {lastSignature ? (
          <a
